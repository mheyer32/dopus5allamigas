...
Cfg_FiletypeList *creator_generic(
	char *args,
...
__stdargs my_SimpleRequestTags(
...
int file_exists( BPTR pathlock, char *path, char *file, ULONG flags )
{
int ok = FALSE;
...
int exists_req( struct Window *win, char *title, char *path, char *name )
{
int ok = FALSE;
...
int Att_FindBestNodeNumber( Att_List *list )
{
Att_Node *test_node;
long count;
...
int finder_openwindow( finder_data *data )
{
int ok = TRUE;
...
void finder_closewindow( finder_data *data )
{
// Remove AppWindow
if	(data->app_window)
{
...
int finder_add_match_to_list( finder_data *data, Cfg_Filetype *ft, int installed )
{
Att_Node *node;
...
int finder_filetypes_in_cache( finder_data *data )
{
Att_Node *		node;
...
int finder_init_listview( finder_data *data )
{
int ok = FALSE;
...
int finder_free_listview( finder_data *data )
{
int ok = FALSE;
...
void finder_print_blurb( finder_data *data, LONG s )
{
int i;
...
int finder_rescan( finder_data *data )
{
int ok = FALSE;
...
int finder_save_edited_filetype( finder_data *data )
{
...
int finder_event_loop( finder_data *data )
{
int cont = 1;
...
int finder_build_cache( finder_data *data )
{
		...
		for	(ftl = (Cfg_FiletypeList *)((struct ListLock *)data->pointer_packet.pointer)->list.lh_Head;
			...
			ftl = (Cfg_FiletypeList *)ftl->node.ln_Succ)
			{
				...
				if (copy=AllocMemH(data->filetype_cache->memory,sizeof(Cfg_FiletypeList)))
				{
					...
					/* Copy filetypes */
					for (type=(Cfg_Filetype *)ftl->filetype_list.lh_Head;
						type->node.ln_Succ;
...
void finder_free_cache( finder_data *data )
{
Att_Node *		node;
...
int creator_openwindow( creator_data *data )
{
int ok = TRUE;
...
void creator_closewindow( creator_data *data )
{
// Remove AppWindow
if	(data->app_window)
{
...
int creator_make_recognition( creator_data *data )
{
int len = 0;
unsigned char *p;
...
int creator_create_filetype( creator_data *data )
{
int ok = FALSE;
...
int creator_get_iff( struct filetype_info *fti, BPTR lock )
{
int ok = FALSE;
BPTR newlock, fh;
LONG buf[3];
...
int creator_get_datatype( struct filetype_info *fti, BPTR lock )
{
int ok = FALSE;
...
int creator_get_bytes( struct filetype_info *fti, BPTR lock )
{
int ok = FALSE;
BPTR newlock, fh;
...
void creator_add_filename( struct filetype_info *master, struct filetype_info *ft )
{
int oldlen, r;
...
void creator_add_bytes( struct filetype_info *master, struct filetype_info *ft )
{
...
void creator_add_bytesc( struct filetype_info *master, struct filetype_info *ft )
{
...
void creator_addnode( creator_data *data, struct filetype_info *fti )
{
ULONG flags;
...
void fti_bytes_print_text( char *buf, struct filetype_info *fti, int nocase )
{
int i;
int c;
char *p = buf;
...
void fti_bytes_print_hex( char *buf, struct filetype_info *fti )
{
int i;
char *p = buf;
UBYTE *mask;
...
int fti_bytes_isprint( struct filetype_info *fti )
{
int i;
int retval = 0;
UBYTE *mask;
...
void fti_bytes_print( char *buf, struct filetype_info *fti, int nocase )
{
...
void creator_display_info( creator_data *data )
{
...
int creator_warn_edited( creator_data *data )
{
int ok = FALSE;
...
int creator_add_file( creator_data *data )
{
struct filetype_info *fti;
Att_Node *node;
...
int creator_delete_file( creator_data *data )
{
int v;
Att_Node *node;
int ok = FALSE;
...
int creator_clear_files( creator_data *data )
{
...
int creator_save_filetype( creator_data *data )
{
int ok = FALSE;
...
int creator_handle_appwindow( creator_data *data )
{
struct AppMessage *amsg;
int a;
...
int creator_event_loop( creator_data *data )
{
struct IntuiMessage *msg;
IPCMessage *imsg;
int cont = 1;
