...
Buttons *buttons_new(char *name, Cfg_ButtonBank *bank, struct IBox *box, short type, ULONG flags)
{
	Buttons *buttons;
	IPCData *ipc;
	...
	// Filename supplied?
		...
		strcpy(buttons->buttons_file, name);

	// Create new bank
	else if (!(buttons->bank = NewButtonBank(1, type)))
	{
		FreeVec(buttons);
		return 0;
	}
