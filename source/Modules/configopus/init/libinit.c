...
// first function should be FAS (without _) or, if it removed function (like RemovedFunc() from dopus5.library), then at
// least LibNull.

#define libvector                                                                                    \
	LFUNC_FAS(L_Config_Settings)                                                                     \
	LFUNC_FA_(L_Config_Environment)                                                                  \
	, LibNull LFUNC_FA_(L_Config_Buttons) LFUNC_FA_(L_ConvertConfig) LFUNC_FA_(L_Config_Filetypes)   \
		  LFUNC_FA_(L_Config_ListerButtons) LFUNC_FA_(L_Config_Menu) LFUNC_FA_(L_Config_Menus)       \
			  LFUNC_FA_(L_EditFiletype) LFUNC_FA_(L_Config_EditFunction) LFUNC_FA_(L_ShowPaletteBox) \
				  LFUNC_FA_(L_FunctionExportASCII)
...
static AROS_LD1(struct LibraryHeader *,
				...
				1,
				DOpus);
static AROS_LD0(BPTR, LibClose, struct LibraryHeader *, base, 2, DOpus);
...
static struct LibraryHeader *LIBFUNC LibInit(REG(d0, struct LibraryHeader *lh),
											 REG(a0, BPTR Segment),
...
STATIC CONST struct TagItem libCreateTags[] = {{CLT_DataSize, sizeof(struct LibraryHeader)},
...
STATIC CONST IPTR LibInitTab[] = {sizeof(struct LibraryHeader), (IPTR)LibVectors, (IPTR)NULL, (IPTR)LibInit};
...
{
#elif defined(__AROS__)
static AROS_UFH3(struct LibraryHeader *,
				 LibInit,
				 ...
				 AROS_UFHA(struct ExecBase *, sb, A6))
{
	AROS_USERFUNC_INIT
#else
static struct LibraryHeader *LIBFUNC LibInit(REG(d0, struct LibraryHeader *base),
...
static struct LibraryHeader *LibOpen(void)
{
	struct LibraryHeader *base = (struct LibraryHeader *)REG_A6;
#elif defined(__AROS__)
static AROS_LH1(struct LibraryHeader *,
				LibOpen,
...
static BPTR LibClose(void)
{
	struct LibraryHeader *base = (struct LibraryHeader *)REG_A6;
#elif defined(__AROS__)
static AROS_LH0(BPTR, LibClose, struct LibraryHeader *, base, 2, DOpus)
{
	AROS_LIBFUNC_INIT
#else
static BPTR LIBFUNC LibClose(REG(a6, struct LibraryHeader *base))
									...
									if ((DiskFontBase = (APTR)OpenLibrary("diskfont.library", 37)) != NULL &&
										...
										if ((IconBase = (APTR)OpenLibrary("icon.library", 37)) != NULL &&
											...
											if ((WorkbenchBase = (APTR)OpenLibrary("workbench.library", 37)) != NULL &&
												...
												if ((DataTypesBase = (APTR)OpenLibrary("datatypes.library", 37)) !=
														NULL &&
