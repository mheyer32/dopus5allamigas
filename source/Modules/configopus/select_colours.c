...
#endif
{
	...
	{
		// Store pens
		data->pen_array[data->pen_count+((((struct Library*)IntuitionBase)->lib_Version>=39)?8:4)]=spec_pen[0];
		...
		if (extra)
		{
			...
			for (b=0;b<2;b++)
			{
				...
				for (a=0;a<3;a++)
				{
					...
					// Set initial colour
					palette_box_colour(data,data->stuff.stuff2.palette_gadget[b],a,ptr,spec_pen[b],ptr[a]>>(32-data->stuff.stuff2.gun_bits[a]));
				}
			}
		}
	}
						...
						case GAD_PALETTE_BACKGROUND:
							{
								...
								if (extra)
								{
									if (fg==data->pen_count+((((struct Library*)IntuitionBase)->lib_Version>=39)?8:4)-1)
										ret|=1<<0;
									if (bg==data->pen_count+((((struct Library*)IntuitionBase)->lib_Version>=39)?8:4)-1)
										ret|=1<<1;
								}
...
void palette_box_colour(PaletteBoxData *data,GL_Object *gadget,short gun,ULONG *spec,short pen,ULONG value)
{
	...
	SetGadgetAttrs(GADGET(gadget),data->window,0,
		DPG_Redraw,data->pen_count+((((struct Library*)IntuitionBase)->lib_Version>=39)?8:4)-1,
		TAG_END);
}
