...
void _env_select_icon_font(config_env_data *data,
						   char *name,
						   UBYTE *size,
						   UBYTE *fpen,
						   UBYTE *bpen,
						   ...
						   short bufsize)
{
	...
	if (AslRequestTags(data->icon_font_req,
					   ...
					   data->window,
					   ASLFO_TitleText,
					   GetString(locale, MSG_SELECT_FONT),
					   ASLFO_InitialName,
					   name,
					   ASLFO_InitialSize,
					   (ULONG)*size,
					   ...
					   (ULONG)*mode,
					   ASLFO_Flags,
					   FOF_PRIVATEIDCMP | FOF_DOFRONTPEN | FOF_DOBACKPEN | FOF_DODRAWMODE,
					   ASLFO_MaxFrontPen,
					   data->font_pen_count,
					   ASLFO_MaxBackPen,
					   data->font_pen_count,
