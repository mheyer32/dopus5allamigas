...
*/
	...
	#define HOOKPROTO(name, ret, obj, param) \
		static SAVEDS ASM ret name(REG(a0, struct Hook *hook), REG(a2, obj), REG(a1, param))
	#define HOOKPROTONO(name, ret, param) static SAVEDS ASM ret name(REG(a0, struct Hook *hook), REG(a1, param))
	#define HOOKPROTONP(name, ret, obj) static SAVEDS ASM ret name(REG(a0, struct Hook *hook), REG(a2, obj))
	#define HOOKPROTONONP(name, ret) static SAVEDS ASM ret name(REG(a0, struct Hook *hook))
	...
	#define HOOKPROTONO(name, ret, param) static SAVEDS ret name(struct Hook *hook, UNUSED APTR _obj, param)
	#define HOOKPROTONP(name, ret, obj) static SAVEDS ret name(struct Hook *hook, obj, UNUSED APTR _param)
	#define HOOKPROTONONP(name, ret) static SAVEDS ret name(struct Hook *hook, UNUSED APTR _obj, UNUSED APTR _param)
	#define HOOKPROTONH(name, ret, obj, param) static SAVEDS ret name(UNUSED struct Hook *_hook, obj, param)
	#define HOOKPROTONHNO(name, ret, param) static SAVEDS ret name(UNUSED struct Hook *_hook, UNUSED APTR _obj, param)
	#define HOOKPROTONHNP(name, ret, obj) static SAVEDS ret name(UNUSED struct Hook *_hook, obj, UNUSED APTR _param)
	#define HOOKPROTONHNONP(name, ret) static SAVEDS ret name(void)
#endif
...
struct SDI_EmulLibEntry
{
	...
	#define MakeHook(hookname, funcname) \
		struct Hook hookname = {{NULL, NULL}, (HOOKFUNC)HookEntry, (HOOKFUNC)funcname, NULL}
	#define MakeHookWithData(hookname, funcname, data) \
		struct Hook hookname = {{NULL, NULL}, (HOOKFUNC)HookEntry, (HOOKFUNC)funcname, (APTR)data}
	#define MakeStaticHook(hookname, funcname) \
		static struct Hook hookname = {{NULL, NULL}, (HOOKFUNC)HookEntry, (HOOKFUNC)funcname, NULL}
	...
	#define DISPATCHER(name)                                                                                          \
		...
		const struct SDI_EmulLibEntry Gate_##name = {SDI_TRAP_LIB, 0, (APTR)Trampoline_##name};                       \
		ULONG name(struct IClass *cl, Object *obj, Msg msg)
	...
	#define MakeHookWithData(hookname, funcname, data) \
		struct Hook hookname = {{NULL, NULL}, (HOOKFUNC)funcname, NULL, (APTR)data}
	#define MakeStaticHook(hookname, funcname) \
		static struct Hook hookname = {{NULL, NULL}, (HOOKFUNC)funcname, NULL, NULL}
	#define ENTRY(func) (APTR) func
	#define DISPATCHERPROTO(name) \
		SAVEDS ASM IPTR name(REG(a0, struct IClass *cl), REG(a2, Object *obj), REG(a1, Msg msg))
	#define DISPATCHER(name) DISPATCHERPROTO(name)
	#define SDISPATCHER(name) static DISPATCHERPROTO(name)

#endif

#define InitHook(hook, orighook, data) \
	((hook)->h_Entry = (orighook).h_Entry, (hook)->h_SubEntry = (orighook).h_SubEntry, (hook)->h_Data = (APTR)(data))
