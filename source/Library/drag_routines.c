...
DragInfo *LIBFUNC L_GetDragInfo(
...
void LIBFUNC L_GetDragImage(
	REG(a0, DragInfo *drag),
	REG(d0, ULONG x),
	REG(d1, ULONG y))
{
	BOOL ok=1;
...
BOOL LIBFUNC L_CheckDragDeadlock(
	REG(a0, DragInfo *drag))
{
	return 0;
}
...
BOOL LIBFUNC L_BuildTransDragMask(
	REG(a0, UWORD *mask),
	REG(a1, UWORD *image),
	REG(d0, short width),
	REG(d1, short height),
...
BOOL LIBFUNC L_DragCustomOk(
	REG(a0, struct BitMap *bm),
	REG(a6, struct MyLibrary *libbase))
{
	BOOL ok=0;
	...
	if (!(data->flags&LIBDF_NO_CUSTOM_DRAG) &&
			((struct Library *)GfxBase)->lib_Version>=39 &&  
			CyberGfxBase &&
