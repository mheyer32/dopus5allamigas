...
void LIBFUNC L_StripWindowMessages(
...
void LIBFUNC L_StoreGadgetValue(
...
void LIBFUNC L_UpdateGadgetValue(
	REG(a0, ObjectList *list),
	REG(a1, struct IntuiMessage *msg),
	REG(d0, UWORD id),
	REG(a6, struct MyLibrary *libbase))
...
void LIBFUNC L_UpdateGadgetList(
	REG(a0, ObjectList *list),
	REG(a6, struct MyLibrary *libbase))
{
	GL_Object *object;
...
BOOL LIBFUNC L_CheckObjectArea(
	REG(a0, GL_Object *object),
	REG(d0, int x),
	REG(d1, int y))
{
	struct IBox *pos;
...
STRPTR LIBFUNC L_GetString(
	REG(a0, struct DOpusLocale *li),
...
struct Gadget *LIBFUNC L_FindKeyEquivalent(
	REG(a0, ObjectList *first_list),
	REG(a1, struct IntuiMessage *msg),
	REG(d0, int process))
{
	GL_Object *object;
...
void LIBFUNC L_ShowProgressBar(
...
int LIBFUNC L_BoundsCheckGadget(
	REG(a0, ObjectList *list),
	REG(d0, ULONG id),
	REG(d1, int min),
	REG(d2, int max))
...
struct IntuiMessage *LIBFUNC L_GetWindowMsg(
	REG(a0, struct MsgPort *port), REG(a6, struct MyLibrary *lib))
{
	struct IntuiMessage *msg;
...
void LIBFUNC L_ReplyWindowMsg(
	REG(a0, struct IntuiMessage *msg))
{
	GT_ReplyIMsg(msg);
}
...
void LIBFUNC L_SetGadgetChoices(
	REG(a0, ObjectList *list),
	REG(d0, ULONG id),
	REG(a1, APTR choices))
{
...
// Refresh a window
void LIBFUNC L_StartRefreshConfigWindow(
	REG(a0, struct Window *window),
...
void LIBFUNC L_EndRefreshConfigWindow(
	REG(a0, struct Window *window))
{
#ifndef BACKFILL
