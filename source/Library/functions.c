...
// void Seed(int seed)
void LIBFUNC L_Seed(REG(d0, int seed))
{
...
UWORD LIBFUNC L_RandomDopus(REG(d0, int limit))
{
...
void LIBFUNC L_BtoCStr(REG(a0, BSTR bstr), REG(a1, char *cstr), REG(d0, int len))
{
	int length = len;
	int size;
#ifdef __AROS__
...
ULONG LIBFUNC L_DivideU(REG(d0, unsigned long num),
