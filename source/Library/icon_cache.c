...
void LIBFUNC L_FreeCachedDiskObject(
...
struct DiskObject *LIBFUNC L_GetCachedDiskObject(
	REG(a0, char *name),
	REG(d0, ULONG flags),
	REG(a6, struct MyLibrary *libbase))
{
	...
	if (NewIconBase && data->NewIconsFlags&ENVNIF_ENABLE)
	{
		...
		// Load as NewIcon
		if (!(ndo=GetNewDiskObject((UBYTE *)name))) return 0;
...
struct DiskObject *LIBFUNC L_GetCachedDiskObjectNew(
	REG(a0, char *name),
	REG(d0, ULONG flags),
	REG(a6, struct MyLibrary *libbase))
{
...
unsigned long LIBFUNC L_IconCheckSum(
	REG(a0, struct DiskObject *icon),
	REG(d0, short which))
{
	unsigned long sum=0;
...
// Set NewIcons flags
void LIBFUNC L_SetNewIconsFlags(
	REG(d0, ULONG flags),
