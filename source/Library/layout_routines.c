...
struct Window *LIBFUNC L_OpenConfigWindow(
	REG(a0, NewConfigWindow *newwindow),
...
void LIBFUNC L_CloseConfigWindow(
	...
	REG(d0, ULONG flags))
{
	...
	if (flags&(WINDOW_SIZE_BOTTOM|WINDOW_SIZE_RIGHT))
	{
		...
		if ((dr=GetScreenDrawInfo(screen)))
		{
			...
			// Get size image
			if ((image=(struct Image *)NewObject(0,"sysiclass",
				SYSIA_DrawInfo,dr,
...
ObjectList *LIBFUNC L_AddObjectList(
	REG(a0, struct Window *window),
...
void LIBFUNC L_FreeObject(
	REG(a0, ObjectList *objlist),
...
void LIBFUNC L_FreeWindowMenus(
	REG(a0, struct Window *window))
{
...
void LIBFUNC L_RefreshObjectList(
	REG(a0, struct Window *window),
