...
long LIBFUNC L_IFFPushChunk(
	REG(a0, IFFHandle *handle),
	REG(d0, ULONG id))
{
	// Valid handle?
	if (!handle) return 0;
...
long LIBFUNC L_IFFWriteChunkBytes(
	REG(a0, IFFHandle *handle),
	REG(a1, char *data),
	REG(d0, long size))
{
...
unsigned long LIBFUNC L_IFFNextChunk(
	REG(a0, IFFHandle *handle),
...
long LIBFUNC L_IFFReadChunkBytes(
	REG(a0, IFFHandle *handle),
	REG(a1, APTR buffer),
	REG(d0, long size))
{
