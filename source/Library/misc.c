...
struct IFFHandle *LIBFUNC L_OpenIFFFile(
	REG(a0, char *name),
	REG(d0, int mode),
...
ULONG LIBFUNC L_SetLibraryFlags(
	REG(d0, ULONG flags),
	REG(d1, ULONG mask),
	REG(a6, struct MyLibrary *libbase))
{
...
ULONG LIBFUNC L_GetLibraryFlags(
	REG(a6, struct MyLibrary *libbase))
{
	struct LibData *data;
...
ULONG LIBFUNC L_CompareListFormat(
	REG(a0, ListFormat *format1),
...
struct Hook *LIBFUNC L_LockReqBackFill(
	REG(a0, struct Screen *screen),
	REG(a6, struct MyLibrary *libbase))
{
	struct LibData *data;
...
void LIBFUNC L_UnlockReqBackFill(
	REG(a6, struct MyLibrary *libbase))
{
	struct LibData *data;
