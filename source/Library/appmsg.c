...
DOpusAppMessage *LIBFUNC L_AllocAppMessage(
	REG(a0, APTR memory),
...
void LIBFUNC L_ReplyAppMessage(
	REG(a0, DOpusAppMessage *msg))
{
	// Valid message?
	if (msg)
...
DOpusAppMessage *LIBFUNC L_CopyAppMessage(
	REG(a0, DOpusAppMessage *orig),
	REG(a1, APTR memory))
{
	DOpusAppMessage *msg;
	short arg;
