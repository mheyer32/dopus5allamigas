...
int search_file(APTR file, UBYTE *search_text, ULONG flags, UBYTE *buffer, ULONG buffer_size)
{
	...
	if (search_text[0] == '$')
	{
		...
		for (buf_pos = 1, match_size = 0; buf_pos < search_len; buf_pos += 2, match_size++)
		{
			...
			// Else do hex conversion
				...
				match_text[match_size] = Atoh(&search_text[buf_pos], 2);
		}
	...
	FOREVER
	{
		...
		// Read some data
		if ((size = ReadBuf(file, buffer, 32000)) < 1)
			break;
...
// Search a buffer
int search_buffer(search_handle *handle,
				  UBYTE *buffer,
				  ULONG buffer_size,
				  UBYTE *match_buf,
				  ULONG match_size,
				  ULONG flags)
{
	...
	int buf_pos, match_pos = 0, match_start = 0;
	int old_lines, old_pos;
