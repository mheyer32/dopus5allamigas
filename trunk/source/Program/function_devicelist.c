		...
		for (type=0;type<num;type++)
		{
			...
			while ((list_entry=NextDosEntry(list_entry,type_order[type])))
			{
				...
				if (list_entry->dol_Type==DLT_DIRECTORY || list_entry->dol_Task)
				{
					...
					// Get pathname
					if (full) DevNameFromLockDopus(list_entry->dol_Lock,handle->work_buffer,256);
				...
				node=node->ln_Succ)
			{
				...
				else
				{
					...
					else
					{
						...
						if (full)
						{
							lsprintf(handle->work_buffer+strlen(handle->work_buffer),"\t\b%ld\b%s",asn_first_x,(IPTR)((char *)(node+1))+32);
						}
					}
				}
