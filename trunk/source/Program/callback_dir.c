...
DirEntry *ASM SAVEDS HookCreateFileEntry(
...
void ASM SAVEDS HookUnlockFileList(
	REG(a0, Lister *lister))
{
	buffer_unlock(lister->cur_buffer);
}

DirEntry *ASM SAVEDS HookFindFileEntry(
	REG(a0, Lister *lister),
	REG(a1, char *name))
{
	DirEntry *entry;
...
BOOL ASM SAVEDS HookSetFileComment(
	REG(a0, Lister *lister),
	REG(a1, char *name),
	REG(a2, char *comment))
{
...
void ASM SAVEDS HookSortFileList(
	REG(a0, Lister *lister),
...
DirEntry *ASM SAVEDS HookAddFileEntry(
	REG(a0, Lister *lister),
	REG(a1, DirEntry *entry),
	REG(d0, BOOL sort))
{
...
void ASM SAVEDS HookResortLister(
	REG(a0, Lister *lister),
...
void ASM SAVEDS HookRefreshLister(
	REG(a0, Lister *lister),
	REG(d0, ULONG flags))
{
	ULONG refresh;
...
void ASM SAVEDS HookRemoveFileEntry(
	REG(a0, Lister *lister),
	REG(a1, DirEntry *entry))
{
