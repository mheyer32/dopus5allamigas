...
void SetupDebug(void)
{
  ...
  _DBPRINTF("** dopus5.library %s (%s) startup ****************************\n", LIB_REV_STRING, LIB_DATE);
  _DBPRINTF("Initializing runtime debugging:\n");

  if(GetVar("dopus5.library.debug", var, sizeof(var), 0) > 0)
  {
    char *s = var;
  ...
  _DBPRINTF("set debug classes/flags (env:dopus5.library.debug): %08lx/%08lx\n", debug_classes, debug_flags);
