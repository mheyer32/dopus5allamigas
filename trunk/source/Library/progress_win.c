...
void LIBFUNC L_CloseProgressWindow(
	REG(a0, ProgressWindow *prog))
{
	// Valid window?
	if (prog)
	{
...
void LIBFUNC L_HideProgressWindow(
	REG(a0, ProgressWindow *prog))
{
	// Valid window?
	if (prog)
...
void LIBFUNC L_ShowProgressWindow(
	REG(a0, ProgressWindow *prog),
...
void LIBFUNC L_SetProgressWindow(
	REG(a0, ProgressWindow *prog),
...
void LIBFUNC L_GetProgressWindow(
	REG(a0, ProgressWindow *prog),
	REG(a1, struct TagItem *tags))
{
	// Valid window?
...
BOOL LIBFUNC L_CheckProgressAbort(
	REG(a0, ProgressWindow *prog))
{
	BOOL abort=0;
...
void progress_open(ProgressWindow *prog)
{
	...
	{
		// Create abort gadget
		if ((prog->pw_Abort=(struct Gadget *)NewObject(
			0,
			"dopusbuttongclass",
...
void progress_set(ProgressWindow *prog,struct TagItem *tags)
{
	...
	if ((ptr=(char *)GetTagData(PW_Title,0,tags)))
	{
		...
		if (prog->pw_Window) SetWindowTitles(prog->pw_Window,prog->pw_Title,(CONST_STRPTR)-1);
	}
