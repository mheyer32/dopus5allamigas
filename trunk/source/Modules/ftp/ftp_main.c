...
if	((path_handle = DC_CALL2(infoptr, dc_GetSource, 
		DC_REGA0, function_handle,
		DC_REGA1, 0)))
	{
		handle = DC_CALL1(infoptr, dc_GetLister, DC_REGA0, path_handle);
		DC_CALL2(infoptr, dc_EndSource, 
			DC_REGA0, function_handle,
			DC_REGD0, 0);
	}
	...
	//if	(h->dc_GetSource( function_handle, 0 ))
		...
		if	((fm = AllocVec( sizeof(struct ftp_msg) + num * sizeof(struct entry_info) + strlen(argv[2]) + 1, MEMF_CLEAR )))
			...
			//while	((entry = h->dc_GetEntry( function_handle )))
				{
				ei.ei_type = DC_CALL2(h, dc_ExamineEntry,
					DC_REGA0, entry,
					DC_REGD0, EE_TYPE);
				...
				if	((entry2 = DC_CALL1(h, dc_ConvertEntry, DC_REGA0, entry)))
					DC_CALL3(h, dc_FileQuery,
						DC_REGA0, (ULONG)node->fn_handle,
						DC_REGA1, entry2,
						DC_REGA2, tags);
				else
					stccpy( ei.ei_name, (char *)DC_CALL2(h, dc_ExamineEntry, DC_REGA0, entry, DC_REGD0, EE_NAME), FILENAMELEN + 1 );
