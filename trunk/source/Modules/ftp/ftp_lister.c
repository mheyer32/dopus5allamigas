...
void lister_add(
	...
	char                   *comment )
{
static D_S(struct FileInfoBlock,fib)
APTR entry;
LONG etype;
...
if	(fm->fm_entries)
	...
	if	((source = create_endpoint_tags(
		...
		if	(quiet || !(options & DELETE_ASK))
			...
			// Show requester
			confirm_commence = lister_request_tags(
				ftpnode,
				FR_FormatString,"%s\n%s:\n\n%ld %s\n%ld %s\n%s %s?",
		...
		if	(confirm_commence)
			...
			for	(
				...
				if	(!quiet && ei->ei_type >= 0 && options & DELETE_DIRS)
					{
					confirm_entry = lister_request_tags(
						ftpnode,
						FR_FormatString,"'%s' %s",
...
if	((names = fm->fm_names))
	...
	if	((source = create_endpoint_tags(
		...
		// Requester
		if	((reqresult = lister_request_tags(
			ftpnode,
